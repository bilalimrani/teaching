$(document).ready(()=>{
	...
	function search(page, teacherlist){
		...
		$.post(`${base_url}/search`, {keyword : keyword, city : city, state : state, zip : zip, page : page, "teacher-list":teacherlist}, (res)=>{
			...
			localStorage.setItem('user_img_url', res.response.teacher_img_url)
			...
			if(res.responseCode == 200){
				...
				if(res.response.schools.data.length > 0){
					$.each(res.response.schools.data, (key, value)=>{
						...
						<p>Address: ${value.address == null || value.address == "" ? "N/A" : value.address}</p>
						<p>Phone: ${value.number == null || value.number == "" ? "N/A" : value.number}</p>
						<p>School Website: ${value.url == null || value.url == "" ? "N/A" : value.url}</p>
				...
				if(res.response.teachers.data.length > 0){
					$.each(res.response.teachers.data, (key, value)=>{
						...
						<p class="t-heading">Grade:</p> <p class="t-content">&nbsp;${value.grade == null || value.grade == "null" ? "N/A" : JSON.parse(value.grade)}</p>
						<p class="t-heading">Subject:</p> <p class="t-content">${value.subject == null || value.subject == "" ? "N/A" : value.subject}</p>
						<p class="t-heading">Email:</p> <p class="t-content email">${value.email == null || value.email == "" ? "N/A" : value.email}</p>
						<p class="t-heading">School:</p> <p class="t-content">${value.schools.name == null || value.schools.name == "" ? "N/A" : value.schools.name}</p>
						<div class="search-buttons-layout">
						<button type="button" class="btn btn-secondary btn-lg btn-student-grading"><p class="btn-S-content">Student <br>grading <p class="btn-S-grade">${value.sGrade == null || value.sGrade == "" ? "N/A" : value.sGrade}</p></p></button>
						<button type="button" class="btn btn-secondary btn-lg btn-teacher-grading"><p class="btn-S-content">Parent <br>grading <p class="btn-S-grade">${value.pGrade == null || value.pGrade == "" ? "N/A" : value.pGrade}</p></p></button>
						</div>
						</div>
						</div>
						`;
