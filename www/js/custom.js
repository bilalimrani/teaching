...
function openCity(evt, cityName,title, prev = null) {
  ...
  }else if(prev == "gotoschooldetail"){
    localStorage.setItem('backstate', 'School-detail');
  } 
  else {
...
$('#backButton').on('click', function() {
  if ($("#Teacher-detail").css('display') == 'block') {
    if (localStorage.getItem('backstate') == "Search-result") {
      ...
      $('#litopteachers')[0].click();
      openCity(event, 'Top-teachers', 'Top Teachers')
    }else if(localStorage.getItem("backstate") == "School-detail"){
    ...
    openCity(event, 'Search-result', 'Search Results')
  }else if($("#Teacher-detail").css('display') == 'block'){
      console.log('i have to detail');
  }
});
...
$(".gradelevel_popup li").on('click',function(e) {  
  ...
  if(flag == true){
    ...
    var da = $(this).find('p').text().replace(/\s*,\s*/g, ",");
    console.log("das",da)
